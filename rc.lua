...
filemanager = "thunar"
editor = "sublime"
lock = "i3lock -i /home/jsanchezpando/.i3/keep-calm-and-make-coffee.png -n"
...
function run_startup(command)
  local proc = string.gmatch(command, "%S+")()
  running = os.execute("ps -eF | grep -v grep | grep -w " .. proc)
  if (running == nil) then
    awful.util.spawn(command, false)
  end
end

for k = 1, #startup_procs do
  run_startup(startup_procs[k])
end
...
if beautiful.wallpaper then
    for s = 1, screen.count() do
        gears.wallpaper.fit(beautiful.wallpaper[s], s)
    end
end
-- }}}
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "Up",     awful.client.restore),

    -- Prompt
    awful.key({ modkey },            "r",     function () awful.util.spawn(launcher) end),
    awful.key({ modkey, "Shift" },   "r",     function () awful.util.spawn(filemanager) end),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Control" }, "Up",     function (c) awful.client.setmaster(c) end),
    awful.key({ modkey,           }, "m",
        function (c)
